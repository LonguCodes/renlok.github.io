import { TYPES } from './data/typeModifiers';
...
const Combat = {
	...
	playerTimer: function () {
		...
		this.playerTimerId = window.setTimeout(
			() =>
				this.dealDamage(
					this.playerActivePoke,
					this.enemyActivePoke,
					'player'
				),
			nextAttack
		);
	...
	calculateDamageMultiplier: function (attackingTypes, defendingTypes) {
		const typeEffectiveness = (attackingType, defendingTypes) =>
			TYPES[attackingType][defendingTypes[0]] *
			((defendingTypes[1] && TYPES[attackingType][defendingTypes[1]]) ||
				1);
		return Math.max(
			typeEffectiveness(attackingTypes[0], defendingTypes),
			(attackingTypes[1] &&
				typeEffectiveness(attackingTypes[1], defendingTypes)) ||
				0
		);
	},
	dealDamage: function (attacker, defender, who) {
		...
		if (attacker.alive() && defender.alive()) {
			const consoleColor =
				who === 'player' ? 'green' : 'rgb(207, 103, 59)';
			// calculate damage done
			const missRNG = RNG(5);
			if (missRNG) {
				dom.gameConsoleLog(
					attacker.pokeName() + ' missed!',
					consoleColor
				);
			} else {
				const critRNG = RNG(5);
				...
				const damage = defender.takeDamage(
					attacker.avgAttack() * damageMultiplier
				);
				if (critRNG) {
				...
				if (who === 'player') {
					// TODO add some flair
					dom.gameConsoleLog(
						attacker.pokeName() + ' Attacked for ' + damage,
						'green'
					);
	...
	enemyFaint: function () {
		...
		const expToGive =
			this.enemyActivePoke.baseExp() / 16 +
			this.enemyActivePoke.level() * 3;
		player.statistics.totalExp += expToGive;
		this.playerActivePoke.giveExp(expToGive);
		dom.gameConsoleLog(
			this.playerActivePoke.pokeName() +
				' won ' +
				Math.floor(expToGive) +
				'xp',
			'rgb(153, 166, 11)'
		);
		player
			...
			.forEach((poke) =>
				poke.giveExp(
					this.enemyActivePoke.baseExp() / 100 +
						this.enemyActivePoke.level() / 10
				)
			);
		...
		if (this.trainer) {
			...
			if (this.trainerPoke.length < 1) {
				dom.gameConsoleLog(
					'You have defeated ' + this.trainer.name,
					'blue'
				);
	...
	newEnemy: function () {
		if (combatLoop.trainer) {
			...
			enemy.generateNew(
				player.settings.currentRegionId,
				player.settings.currentRouteId
			);
		}
		this.enemyActivePoke = enemy.activePoke();
		player.addPokedex(
			enemy.activePoke().pokeName(),
			enemy.activePoke().shiny()
				? POKEDEXFLAGS.seenShiny
				: POKEDEXFLAGS.seenNormal
		);
		if (enemy.activePoke().shiny()) {
	...
	playerFaint: function () {
		...
		if (alivePokeIndexes.length > 0) {
			...
			dom.gameConsoleLog(
				'You have no more usable pokemon. You blacked out!',
				'red'
			);
			if (this.trainer) {
			...
			flash($('#gameContainer'));
			dom.gameConsoleLog(
				'You reawaken at the nearest pokecenter.',
				'blue'
			);
			if (
	...
	attemptCatch: function () {
		if (
			this.catchEnabled == 'all' ||
			(this.catchEnabled == 'new' &&
				!player.hasPokemon(enemy.activePoke().pokeName(), 0)) ||
			enemy.activePoke().shiny()
		) {
			dom.gameConsoleLog(
				'Trying to catch ' + enemy.activePoke().pokeName() + '...',
				'purple'
			);
			const selectedBall = enemy.activePoke().shiny()
				? player.bestAvailableBall()
				: player.selectedBall;
			if (player.consumeBall(selectedBall)) {
				...
				const rngHappened = RNG(
					((enemy.activePoke().catchRate() *
						player.ballRNG(selectedBall)) /
						3) *
						catchBonus
				);
				if (rngHappened) {
					...
					dom.gameConsoleLog(
						'You caught ' + enemy.activePoke().pokeName() + '!!',
						'purple'
					);
					...
					player.addPokedex(
						enemy.activePoke().pokeName(),
						enemy.activePoke().shiny()
							? POKEDEXFLAGS.ownShiny
							: POKEDEXFLAGS.ownNormal
					);
					if (enemy.activePoke().shiny()) {
					...
					dom.gameConsoleLog(
						enemy.activePoke().pokeName() + ' escaped!!',
						'purple'
					);
				}
			}
	...
	findPokeballs: function (pokeLevel) {
		...
		for (let ballName in ballWeights) {
			if (rng < ballWeights[ballName]) {
				...
				dom.gameConsoleLog(
					'You found ' + ballsAmount + ' ' + ballName + 's!!',
					'purple'
				);
				dom.renderBalls();
			}
	...
	changeCatch: function (shouldCatch) {
		this.catchEnabled = shouldCatch;
	}
};
